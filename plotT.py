...
def plotT(dataset, outputFileOutlet, outputFileInside, outputFileSensor, plotFileName):
    ...
    # Plot the Temperature temperatures for the common area
    plt.figure(figsize=(12, 6))

    # Plot Outlet NOAA Temperature
    plt.plot(common_outlet['Timestamp'], common_outlet['Temperature'], label='NOAA Outlet RH', color='blue')

    # Plot Wet Neph NOAA Temperature
    plt.plot(common_wet_neph['Timestamp'], common_wet_neph['Temperature'], label='NOAA Wet Neph RH', color='orange')

    # Plot Sensor Temperature
    plt.plot(common_sensor['Timestamp'], common_sensor['Temperature'], label='Inlet Sensor RH', color='green')

    # Formatting the plot
    plt.title('Temperature Comparison (Common Area)')
